...
It is a compact benchmark graph: the image preparation and reference labels are shared, while Cellpose3, Cellpose-SAM, StarDist, InstanSeg, and a classical threshold method are evaluated side by side.
...
.. raw:: html
   ...
   flowchart LR
     ...
     prepare --> instanseg[instanseg_segmentation]:::method
...
7. ``instanseg_segmentation`` runs the official ``fluorescence_nuclei_and_cells`` model with the nuclei target.
8. ``classical_threshold_segmentation`` runs a threshold-based segmentation method.
9. The benchmark nodes compare each predicted label image with the reference labels and write one overlay per method.
10. ``bbbc038_benchmark_metrics`` concatenates the per-method metric rows into the final table.
...
.. figure:: images/bbbc038_segmentation_benchmark/bbbc038_method_overlays.png
   :alt: Segmentation overlays for one BBBC038 sample.

   Output overlay preview for one BBBC038 image.
   The existing preview shows Cellpose3, Cellpose-SAM, StarDist, and the classical threshold method; workflow results also contain the InstanSeg overlay.
