...
Wetlands 2 rejects channel-qualified Conda dependencies, so BioImageFlow passes `atlas>=0` and `("bioimageit", "conda-forge")` for `bioimageit::atlas>=0` with no declared channels. This makes both channels available to solve Python and package dependencies but does not pin `atlas` to `bioimageit`: another configured channel could supply it. True per-package source pinning requires coordinated Wetlands support for channel-qualified requirements or an equivalent source constraint.
...
Ordinary retry leaves current selections intact; recompute invalidates selected current pointers and optionally transitive downstream selections but never deletes immutable records.
Uncertain scheduler submission consumes the prepared object and never triggers automatic resubmission; recovery reconnects to the plan's child run ID.
...
from bioimageflow import (
    ...
    ResourceLifetime, WorkflowExecutionContext, ExecutionProviderOutcome,
    WetlandsEnvManager,
