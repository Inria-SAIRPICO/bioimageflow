...
For a specific workflow, GUIs must also register its workflow-local custom tools:
...
`register_workflow` discovers only custom tools carried by that workflow. It does not install or register package tools; call `register_package` for package references. For exported dicts, discovery uses the `custom_tool_modules` bundle written by `Workflow.export()`.
...
### 3.12 Workflow-Local Custom Tools

A workflow may define custom `ProcessingTool`, `DataFrameTool`, or class-based `SubWorkflow` classes in a project-local `tools/` package. These tools are owned by that workflow or project and are exported with the workflow file; they do **not** need to be promoted to a versioned tool package merely to make a workflow shareable.
...
- Tool classes live in `tools/*.py` and are re-exported from `tools/__init__.py` for readable workflow imports.
- Each custom tool module should include the code needed by that tool class, or import normal installed dependencies. `Workflow.export()` bundles the module that defines the custom tool class.
...
- Static assets may be committed when they are small and intrinsic to the tool. Large binaries, trained models, and generated artifacts must be declared as dependencies, downloaded at runtime, or generated by the workflow.

Export behavior:

- `Workflow.export(path)` calls `to_dict(include_custom_tools=True)` and writes a top-level `custom_tool_modules` list containing source, source hash, original module name, and filename for each workflow-local custom module used by a node.
- Regular tool nodes reference an embedded module with `tool_source_module`; class-based sub-workflow nodes use `sub_workflow_source_module`.
- `Workflow.load(path)` validates the embedded source hash, materializes the source into a temporary Python module, and resolves the class from that module before attempting package or normal import resolution.
- `ToolRegistry.register_workflow(workflow_or_data)` discovers workflow-local custom tools from either a live `Workflow` or an exported workflow dict, so GUI tool discovery for a selected workflow includes that workflow's custom tools.
...
- Workflow-local custom tool source modules, when a node uses a custom tool that is not part of a versioned tool package.
...
Versioned package tool code is **not** serialized — the same tool packages (at the referenced versions) must be available in the tool store to re-execute a loaded workflow. Workflow-local custom tools are serialized into a top-level `custom_tool_modules` bundle by `Workflow.export()`, and nodes reference that bundle with `tool_source_module` or `sub_workflow_source_module`. `Workflow.load()` uses the embedded source before falling back to package or import resolution, so exported workflows carry their custom tools across machines.
...
# Include the workflow-local custom-tool bundle in memory.
