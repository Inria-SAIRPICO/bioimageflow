...
Use it after segmentation when object geometry is needed for QC or downstream joins.
Non-2D label images raise a `ValueError`.
...
One output row is returned for each positive label, with exact pixel area, centroid coordinates, and bounding-box coordinates.
