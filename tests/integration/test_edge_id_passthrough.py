...
def test_edge_id_round_trips(tmp_path: Path) -> None:
...
def test_edge_id_is_required(tmp_path: Path) -> None:
    graph = _column_graph(tmp_path)
...
def test_dataframe_edges_have_independent_ids(tmp_path: Path) -> None:
    ...
    round_trip = Workflow.from_dict(graph, storage_path=tmp_path).to_dict()
