"""
Shared stub tool definitions for integration tests.
...
class StubSegmenter(ProcessingTool):
    ...
    class Inputs(IOModel):
        ...
        diameter: Annotated[
            float, GUIMeta(connectable=Connectable.NEVER, min=1.0, max=500.0, step=0.5)
        ] = 30.0
...
class StubTiler(ProcessingTool):
    ...
    class Inputs(IOModel):
        ...
        tile_count: Annotated[
            int, GUIMeta(connectable=Connectable.NEVER, min=1, max=64, step=1)
        ] = 4
...
class StubBatchProcessor(ProcessingTool):
    ...
    def process_batch(
        self, arguments_list: list[Any], *, context: object | None = None
    ) -> Any:
        results = []
...
class StubRegistration(ProcessingTool):
    ...
    class Outputs(IOModel):
        registered: Annotated[Path, ImageSpec(semantics={Semantic.INTENSITY})] = (
            Template("{fixed.stem}_registered_{row_index}.tif")
        )
        displacement: Annotated[Path, ImageSpec(semantics={Semantic.DISPLACEMENT})] = (
...
class CellposeTrain(CellposeBase):
    ...
    class Inputs(IOModel):
        ...
        training_masks: Annotated[
            Path,
            ImageSpec(semantics={Semantic.LABEL}),
            GUIMeta(connectable=Connectable.BY_DEFAULT),
        ]
        epochs: Annotated[
            int, GUIMeta(connectable=Connectable.NEVER, min=1, max=10000, step=10)
        ] = 100
